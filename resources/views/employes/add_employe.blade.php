...
<div class="min-h-screen flex items-center justify-center bg-gray-100">
    ...
    <div class="flex flex-col md:flex-row w-full max-w-4xl mx-auto p-6 gap-6">
        ...
        <form action="/add_employe" method="POST" class="w-full md:w-1/2 p-6 bg-white rounded-lg shadow-md animate-fade-in h-fit">
            @csrf
            @if(session('success'))
        ...
        <div class="w-full md:w-1/2 p-6 bg-white rounded-lg shadow-md animate-fade-in">
            ...
            <div class="mb-4 flex flex-col sm:flex-row gap-2">
                <select id="sortSelect" class="px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-[#38d62c]">
                    ...
                    <ul id="employeeList" class="space-y-2">
                        @foreach($employes as $employe)
                            <li class="employee-item p-3 bg-gray-50 rounded-md hover:bg-gray-100 transition"
                                ...
                                <span class="text-sm text-gray-500">({{ $employe->id_employe }})</span>
                            </li>
                        @endforeach
                    </ul>
                @endif
